...
func TestPointer(t *testing.T) {
	...
	for i, v := range s {
		b := cast.StringToBytes(v)
		b2 := []byte(v)
		if b.Writeable() {
			b[0] = 'x'
		}
		fmt.Printf("%d\ts=%5s\tptr(v)=%-12v\tptr(StringBytes(v)=%-12v\tptr([]byte(v)=%-12v\n",
			i, v, cast.StringToPointer(v), b.Pointer(), cast.BytesToPointer(b2))
	}
	...
	for i, v := range b {
		s1 := cast.BytesToString(v)
		s2 := string(v)
		fmt.Printf("%d\ts=%5s\tptr(v)=%-12v\tptr(StringBytes(v)=%-12v\tptr(string(v)=%-12v\n",
			i, s1, cast.BytesToPointer(v), s1.Pointer(), cast.StringToPointer(s2))
	}
